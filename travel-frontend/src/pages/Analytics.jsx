...
export default function Analytics() {
  ...
  useEffect(() => {
    ...
    const loadUserAnalytics = async () => {
      ...
      try {
        ...
        setData(d); setIsAdminView(false);
      } catch (e) {
        setError(e.message);
      } finally {
        setLoading(false);
  ...
  const loadAdminAnalytics = async () => {
    ...
    try {
      ...
      if (!res.ok || d.error) throw new Error(d.error || "Failed to load admin analytics");
      setAdminData(d);
      setIsAdminView(true);
    } catch (e) {
      setError(e.message);
  ...
  const activeData = isAdminView ? adminData : data;

  const maxCityCount = activeData?.topCities?.length
    ? Math.max(...activeData.topCities.map(c => c.count))
    : 1;
  const maxMonthly = activeData?.monthly?.length
    ? Math.max(...activeData.monthly.map(m => m.trips))
    : 1;

  // Sort monthly ascending for chart display
  const monthlyAsc = activeData?.monthly
    ? [...activeData.monthly].sort((a,b) => {
      ...
      <style>{`
        ...
        *, *::before, *::after { box-sizing:border-box; margin:0; padding:0; }
        :root {
          ...
          --text-primary:#FFFFFF; --text-muted:rgba(255,255,255,0.40); --text-sub:rgba(255,255,255,0.65);
          --nav-h:68px;
        }
        ...
        .an-emergency-btn {
          padding:9px 18px;border-radius:50px;border:1.5px solid var(--accent);
          background:var(--accent);color:#fff;font-family:'Sora',sans-serif;
          font-size:13px;font-weight:700;cursor:pointer;transition:background 0.2s,transform 0.15s;
          display:flex;align-items:center;gap:7px;
        }
        .an-emergency-btn:hover { background:var(--accent-hover); transform:translateY(-1px); }
      ...
      <nav className="an-nav">
        ...
        <div className="an-nav-links">
         ...
         <div className="an-nav-actions">
          <button className="an-emergency-btn" onClick={() => navigate("/emergency")}>🚨 Emergency</button>
          {user ? (
        ...
        <div className="an-inner">
           ...
           {toast && (
             <div style={{ background:"rgba(232,52,26,0.12)",border:"1px solid rgba(232,52,26,0.3)",
               borderRadius:14,padding:"12px 16px",color:"#ff9a8a",fontSize:13, marginBottom:12 }}>
               {toast}
             </div>
           )}
          ...
          {error && (
           ...
           {!loading && !error && activeData && (
            <>
              <div className="an-header" style={{ display:"flex", justifyContent:"space-between", alignItems:"center", gap:12, flexWrap:"wrap" }}>
                <div>
                  <div className="an-header-title">{isAdminView ? "🏢 Company Overview" : "📊 Your Travel Analytics"}</div>
                  <div className="an-header-sub">{isAdminView ? "Global view (admin)" : "A snapshot of your adventures with Velora"}</div>
                </div>
                <div style={{ display:"flex", gap:10 }}>
                  <button className="an-cta" style={{ background:isAdminView?"var(--surface2)":"var(--accent)", border:isAdminView?"1px solid var(--border)":"none" }}
                    onClick={() => { setIsAdminView(false); setAdminData(null); }}>
                    My Dashboard
                  </button>
                  <button className="an-cta" style={{ background:isAdminView?"var(--accent)":"var(--surface2)", border:isAdminView?"none":"1px solid var(--border)" }}
              ...
              <div className="an-stats">
                <div className="an-stat">
                  ...
                  <div className="an-stat-value accent">{activeData.summary?.totalTrips ?? 0}</div>
                  <div className="an-stat-sub">trips planned</div>
                </div>
                <div className="an-stat">
                  <div className="an-stat-bg">📍</div>
                  <div className="an-stat-label">KM Travelled</div>
                  <div className="an-stat-value gold">
                    {((activeData.summary?.totalDistanceKm) || 0).toFixed(1)}
                  </div>
                  <div className="an-stat-sub">kilometres total</div>
                </div>
                <div className="an-stat">
                  <div className="an-stat-bg">🏙️</div>
                  <div className="an-stat-label">{isAdminView ? "Total Users" : "Top City"}</div>
                  <div className="an-stat-value green">
                    {isAdminView ? (activeData.summary?.totalUsers || 0) : (activeData.topCities?.[0]?.city || "None")}
                  </div>
                  <div className="an-stat-sub">{isAdminView ? "active users" : "most visited"}</div>
                </div>
              </div>

              {/* No data state */}
              {activeData.summary?.totalTrips === 0 && (
                <div className="an-card an-card-full">
                  <div className="an-card-body">
                    <div className="an-empty">
                      <div className="an-empty-icon">🗺️</div>
                      <div className="an-empty-text">No trips recorded yet. Start exploring to unlock your analytics!</div>
              ...
              {activeData.summary?.totalTrips > 0 && (
                <>
                    ...
                    <div className="an-card">
                      ...
                      <div className="an-card-body">
                        <div style={{ display:"flex", flexWrap:"wrap", gap:8, marginBottom:14 }}>
                          {activeData.topCities?.length ? activeData.topCities.map(c => (
                          ...
                          )) : <span style={{ color:"var(--text-muted)", fontSize:12 }}>No cities yet</span>}
                        </div>
                        {activeData.topCities?.length ? (
                          activeData.topCities.map((c, i) => (
                  ...
                  <div className="an-card an-card-full">
                    ...
                    <div className="an-card-body">
                      <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fill,minmax(200px,1fr))", gap:12 }}>
                        {[
                          { icon:"🌱", label:"First Trip",        unlocked: activeData.summary?.totalTrips >= 1,  desc:"Completed your first trip" },
                          { icon:"✈️", label:"Frequent Flier",    unlocked: activeData.summary?.totalTrips >= 5,  desc:"5 trips planned" },
                          { icon:"🗺️", label:"Explorer",          unlocked: activeData.summary?.totalTrips >= 10, desc:"10 trips completed" },
                          { icon:"🏙️", label:"City Hopper",       unlocked: activeData.topCities?.length >= 3,    desc:"Visited 3+ cities" },
                          { icon:"🚀", label:"Globetrotter",      unlocked: activeData.topCities?.length >= 5,    desc:"Explored 5+ cities" },
                          { icon:"📏", label:"100 km Club",       unlocked: (activeData.summary?.totalDistanceKm || 0) >= 100, desc:"100 km covered" },
                        ].map(m => (
                  ...
                  {isAdminView && adminData?.topUsers && (
                    <div className="an-card an-card-full">
                      <div className="an-card-head">
                        <div className="an-card-title">
                          <div className="an-card-icon">🏅</div>
                      ...
                      <div className="an-card-body" style={{ display:"grid", gridTemplateColumns:"repeat(auto-fill,minmax(240px,1fr))", gap:12 }}>
                        {adminData.topUsers.map((u,i)=>(
                          <div key={u._id||i} style={{
                            background:"var(--surface2)", border:"1px solid var(--border)", borderRadius:14,
                            padding:"12px 14px"
                          }}>
                            <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:6 }}>
                              <span style={{ fontWeight:800, color:"#fff" }}>{i+1 <=3 ? ["🥇","🥈","🥉"][i] : `#${i+1}`}</span>
                              <span style={{ fontSize:12, color:"var(--text-muted)" }}>{u.tripCount} trips</span>
                            </div>
                            <div style={{ fontSize:14, fontWeight:700, color:"#fff" }}>{u.name || "User"}</div>
                            <div style={{ fontSize:12, color:"var(--text-muted)", marginTop:2 }}>{u.email || "N/A"}</div>
                          </div>
                        ))}
                      </div>
                    </div>
